...
// Provide the connection string via PROVISION_TEST_PG_URI, e.g.:
//
//	PROVISION_TEST_PG_URI=postgres://mina:pw@localhost:5432/archive \
...
func testPgURI(t *testing.T) string {
	t.Helper()
	uri := os.Getenv("PROVISION_TEST_PG_URI")
	if uri == "" {
		t.Skip("PROVISION_TEST_PG_URI not set; skipping live Postgres setup test")
	}
	return uri
}
